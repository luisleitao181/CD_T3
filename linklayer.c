    ...
    while(readstate!=6 && readstate!=7){
        ...
        switch(readstate){
            ...
            case 4:///BCC1  e receber dados
                ...
                if (readbuffer[0] ==FLAG ^ XOR_OCTET && message[messagecount-1]== ESCAPE_OCTET){
                    message[messagecount-1]== FLAG;
                } 
                else if(readbuffer[0] ==ESCAPE_OCTET ^ XOR_OCTET && message[messagecount-1]== ESCAPE_OCTET){
...
int llwrite(char* buf, int bufSize)
{
    ...
    for (int i = 0; i < bufSize; i++) {/////////stuffing
        ...
        if (buf[i] == FLAG) {
            ...
            outbuf[out_len++] = FLAG ^ XOR_OCTET;
        } else if (buf[i] == ESCAPE_OCTET) {
            outbuf[out_len++] = ESCAPE_OCTET;
            outbuf[out_len++] = ESCAPE_OCTET ^ XOR_OCTET;
        } else {
